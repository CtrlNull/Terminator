...
bot.login(BOTCONF.token).then(() => {
    ...
    if (ROBPlayground && ROBPlayground.channels.get(CONSTS.robPlayground['channels'][0]['muzak'])) {
        ROBPlayground.channels.get(CONSTS.robPlayground['channels'][0]['muzak']).send("10 minutes until TERMINATION").then(() => console.log("RobPlayground Init"));
    } else {
    ...
    if (TEKy && TEKy.channels.get(CONSTS.tekies['channels'][0]['pr001'])) {
        TEKy.channels.get(CONSTS.tekies['channels'][0]['pr001']).send("10 minutes until TERMINATION").then(() => console.log('Tekies initSent'));
    } else {
        console.log("Failed");
    }
    ...
    if (carolinaServ && carolinaServ.channels.get(CONSTS.theCarolinas['channels'][0]['private001'])) {
        carolinaServ.channels.get(CONSTS.theCarolinas['channels'][0]['private001']).send("10 minutes until TERMINATION").then(() => console.log('The Carolinas warnning sent'));
    } else {
        console.log("Failed");
    }
